...
# var tree_save_data : TreeSaveData = GameSaver.get_save_data_of_type(TreeSaveData) as TreeSaveData


static func get_save_data_of_type(type : Script) -> SaveData:
	# checking if save data exists
	if not FileAccess.file_exists(SAVE_FILE_NAME):
		return null
	...
	# pass data to SaveData
	return type.from_dict(json.data as Dictionary)
